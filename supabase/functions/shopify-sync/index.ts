...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const normalizePayment = (gateway: string, financialStatus: string, transactions: any[] = []) => {
      ...
      // Explicit Cash-on-Delivery detection. This takes PRECEDENCE over any card
      // gateway that may also be present: when a customer's card is refused and
      // they fall back to COD, Shopify keeps the failed card gateway in the list,
      // but the order is unpaid COD. We must NOT read that as paid.
      // (Careful: "vodafone cash"/"orange cash" are online wallets, not COD — the
      // patterns below only match true cash-on-delivery markers.)
      const mentionsCOD =
        gatewayLower.includes('cash on delivery') ||
        gatewayLower.includes('cash_on_delivery') ||
        gatewayLower.includes('cashondelivery') ||
        gatewayLower.includes('(cod)') ||
        /(^|[^a-z])cod([^a-z]|$)/.test(gatewayLower) ||
        gatewayLower.includes('عند الاستلام') ||
        gatewayLower.includes('عند الإستلام')

      // Real captured money = sum of SUCCESSFUL sale/capture transactions (minus
      // refunds). A failed/declined card contributes nothing. This is what tells a
      // "refused card then COD" order (captured = 0 → unpaid COD) apart from a
      // genuine "paid part online + rest COD" (captured > 0 → real partial).
      let capturedPaid = 0
      ...
      // COD takes precedence, but ONLY marks the order paid if money was actually
      // captured. Otherwise online methods are treated as paid (Paymob/Valu often
      // report Shopify financial_status=pending even after a real capture).
      ...
      if (mentionsCOD) {
        if (statusLower.includes('paid') && !isPartiallyPaid) {
          ...
          // Some money really was captured → genuine partial; let admin confirm.
          paymentStatus = ''
        } else {
          // Nothing captured (e.g. a refused card attempt) → plain unpaid COD.
          paymentStatus = 'cod'
        }
      } else if (gatewayLower.includes('valu')) {
        paymentStatus = 'paid'
      ...
      if (mentionsCOD && capturedPaid <= 0 && !(statusLower.includes('paid') && !isPartiallyPaid)) {
        primaryMethod = 'cash'
    ...
    const processOrder = async (shopifyOrder: any, imageMap: Record<string, string> = {}): Promise<{ imported: boolean, updated: boolean }> => {
      ...
      // Check both gateway and ALL payment_gateway_names for payment method detection.
      // We must include the WHOLE array (not just [0]) so that a "Cash on Delivery"
      // gateway is still seen when a failed/declined card attempt sits in front of it.
      const gatewayString = [
      ...
      const orderData: any = {
        ...
        // If we determined the order is an unpaid COD (COD gateway, nothing
        // captured — e.g. a refused card then COD), do NOT store Shopify's
        // misleading 'partially_paid'/'pending'; store 'pending' so the UI doesn't
        // flag it as partially paid.
        financial_status: paymentInfo.status === 'cod'
          ? 'pending'
          : (shopifyOrder.financial_status || paymentInfo.status),
        ...
        partial_paid_amount: (() => {
          const totalPrice = parseFloat(shopifyOrder.current_total_price || shopifyOrder.total_price || 0)
          const txPaid = sumPaidFromTransactions(transactions)
          // Only REAL captured money (a successful transaction) counts as partially
          // paid. Do NOT derive "paid" from total - outstanding: a refused card
          // attempt then COD leaves an outstanding gap that is NOT money received,
          // which previously made unpaid COD orders look partially paid.
      ...
      if (existing && existing.assigned_courier_id) {
        ...
        const updateData: any = {
          ...
          total_order_fees: (() => {
            ...
            const hasRemovedItems = lineItems.some((i: any) => {
              const qtyZero = Number(i?.quantity) === 0
              const propRemoved = i?.properties?._is_removed === true || i?.properties?._is_removed === 'true'
              return i?.is_removed === true || qtyZero || propRemoved
            })
          ...
          order_note: orderData.order_note, // Always update from Shopify
          customer_note: orderData.customer_note, // Always update from Shopify
          notes: orderData.notes, // Always update from Shopify (combines order_note and customer_note)
        ...
        await syncOrderItems(existing.id, shopifyOrder.line_items, shopifyOrder.refunds, parseFloat(shopifyOrder.subtotal_price || shopifyOrder.current_subtotal_price || 0))
      ...
      if (shopifyOrder.line_items) {
        ...
        if (order) {
          await syncOrderItems(order.id, shopifyOrder.line_items, shopifyOrder.refunds, parseFloat(shopifyOrder.subtotal_price || shopifyOrder.current_subtotal_price || 0))
        }
      }
  ...
  // Helper function to sync order items
    const syncOrderItems = async (orderId: string, lineItems: any[], refunds: any[] = [], shopifySubtotal: number = 0) => {
      if (!lineItems) return;
      ...
      // 2. Detect items removed from subtotal (but still in line_items)
      const itemsNotInSubtotal = new Set<string>()
      {
        // Calculate total of all line items
        const allItemsTotal = lineItems.reduce((sum: number, item: any) => {
          return sum + ((parseFloat(item.price || 0) * (item.quantity || 0)) - parseFloat(item.total_discount || 0))
        }, 0)
        
        // Use subtotal detection including zero-subtotal (all items removed)
      ...
      const itemsFromShopify = lineItems
        ...
        if (isRemovedInShopify) {
          console.log(`🔍 Item detected as removed in sync: "${item.title}" (ID: ${item.id})`, {
      ...
      const completelyRemovedItems: any[] = []
      existingItemsMap.forEach((dbItem: any) => {
        ...
        if (dbItem.shopify_line_item_id && !stillExistsInShopify) {
          console.log(`🗑️ Item COMPLETELY REMOVED from Shopify - storing with is_removed=true: ${dbItem.title} (Shopify ID: ${dbItem.shopify_line_item_id})`)
          completelyRemovedItems.push({
            ...dbItem,
            is_removed: true,
            is_new: false,
            quantity: 0,
      ...
      // Include items completely removed from Shopify so UI can display them
      completelyRemovedItems.forEach((item: any) => {
        const key = String(item.shopify_line_item_id)
      ...
      if (allItems.length > 0) {
        ...
        if (itemsToInsert.length > 0) {
          ...
          console.log(`✅ Inserted ${itemsToInsert.length} items (${removedCount} removed, ${newCount} new)`)
        }
      }
    }
